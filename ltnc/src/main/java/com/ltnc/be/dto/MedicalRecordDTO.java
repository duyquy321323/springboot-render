...
public class MedicalRecordDTO {
  ...
  public static MedicalRecordDTO fromDomain(MedicalRecord medicalRecord) {
    return MedicalRecordDTO.builder()
        ...
        .patientName(
            medicalRecord.getPatient().getName()) // Ensure there is a getName method in Patient
        .employeeIds(
            medicalRecord.getEmployees().stream()
                .map(employee -> employee.getId())
                .collect(Collectors.toList()))
        .medicalTests(
            medicalRecord.getMedicalTests().stream()
                .map(MedicalTestDTO::fromDomain)
                .collect(Collectors.toList()))
        .prescriptions(
