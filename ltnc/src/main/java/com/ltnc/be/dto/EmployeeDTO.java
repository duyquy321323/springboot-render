...
public class EmployeeDTO {
  ...
  public static EmployeeDTO fromDomain(Employee employee) {
    return EmployeeDTO.builder()
        ...
        .roomEmployees(
            employee.getRoomEmployees().stream()
                .map(RoomEmployeeDTO::fromDomain)
                .collect(Collectors.toList()))
        .tasks(employee.getTasks().stream().map(TaskDTO::fromDomain).collect(Collectors.toList()))
        .equipments(
            employee.getEquipments().stream()
                .map(EquipmentDTO::fromDomain)
                .collect(Collectors.toList()))
        .prescriptions(
            employee.getPrescriptions().stream()
                .map(PrescriptionDTO::fromDomain)
                .collect(Collectors.toList()))
        .medicalRecords(
            employee.getMedicalRecords().stream()
                .map(MedicalRecordDTO::fromDomain)
                .collect(Collectors.toList()))
        .medicineManagements(
            employee.getMedicineManagementList().stream()
