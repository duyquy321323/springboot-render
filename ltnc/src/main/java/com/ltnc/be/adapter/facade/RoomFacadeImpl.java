...
public class RoomFacadeImpl implements RoomFacade {
  ...
  public void assignPatientToRoom(Long roomId, Long patientId, int bedNumber) {
    ...
    PatientRoom patientRoom =
        new PatientRoom(patient, room, bedNumber, System.currentTimeMillis(), null);
  ...
  public void deleteRoomPatient(Long roomId, Long patientId) {
    ...
    room.getPatientRooms()
        .removeIf(patientRoom -> patientRoom.getPatient().getId().equals(patientId));
  ...
  public void deleteRoomEmployee(Long roomId, Long employeeId) {
    Room room =
        roomRepository.findById(roomId).orElseThrow(() -> new RuntimeException("Room not found"));
    room.getRoomEmployees()
        .removeIf(roomEmployee -> roomEmployee.getEmployee().getId().equals(employeeId));
    roomRepository.save(room);
  }
}
